// This example combines all three database libraries and provides one tool that can brute force
// MySQL, MongoDB, or PostgreSQL. The database type is specified as one of the command-line arguments,
// along with the username, host, password file, and database name.
// MongoDB and MySQL do not require a database name such as PostgreSQL,
// so it is optional when not using the postgres option. A special variable called loginFunc
// is created to store the login function associated with the specified database type.
// The login function is then used to perform the brute force attack
package main
