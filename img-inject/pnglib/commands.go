...
import (
	...
	"github.com/feliux/img-inject/utils"
)

const (
	// Every valid PNG image will have an IEND chunk type indicating the final chunk of the file
	endChunkType = "IEND"
)
...
func (mc *MetaChunk) ProcessImage(b *bytes.Reader, c *types.CmdLineOpts) {
	mc.validate(b)
...
func (mc *MetaChunk) createChunkCRC() uint32 {
	// Recall that the CRC checksum for each chunk segment comprises both the TYPE and DATA byte
	bytesMSB := new(bytes.Buffer)
	if err := binary.Write(bytesMSB, binary.BigEndian, mc.Chk.Type); err != nil {
		log.Fatal(err)
	}
